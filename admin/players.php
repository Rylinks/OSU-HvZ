...
<input type='checkbox' name='show_npcs' value='1' <?php if($show_npcs) print "checked"; ?>> Show NPCs
</center>
...
<?php
	$ret = mysql_query("SELECT * FROM $table_u WHERE $faction ORDER BY $sort_by $order;");
	if($ret && ($rows = mysql_num_rows($ret)) > 0)
	{
		for($i = 0; $i < $rows; $i++) 
		{
			...
			if($admin) print "<td align='center'><a href='delete_player.php?id=".$row['id']."'>delete</a></td>";
			print "</tr>";
		}
	} 
	else 
