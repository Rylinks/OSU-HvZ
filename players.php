<?php
	...
	if($_POST['mode'] == 'refresh') 
	{
		...
		$faction = "WHERE isnpc = 0 AND ".$post_faction_array[$_POST['faction']];
...
<table border="1"  width="100%" cellspacing="0" class="data-table">
<tr>
<?php
...
<?php
	...
	if($ret && ($rows = mysql_num_rows($ret)) > 0)
	{
		...
		for($i = 0; $i < $rows; $i++) 
		{
			...
			if($show_pics) 
			{
				...
				if(strlen($row['pic_path']) > 0) 
				{
					...
					print "<a href=".$picpath.">";
					print '<img src="'.$picpath.'" class="player-pic '.$state_translate[$row['state']].'-pic"  height = "150"></a>';
				} 
				else 
				{
		...
		$playcnt = $rows - $npcs;
		print "<center><br> Number of Players: $playcnt </center>";
	} 
	else 
	{
